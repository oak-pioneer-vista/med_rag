...
Instruction-style query `"Retrieve all patients diagnosed with Lymphoblastic Leukemia"` against the collection, top-50:

- **23 unique docs** among the 50 section hits.
- **Rank 1–2 @ 0.688**: `Antibiotic Therapy Consult` (docs 4540, 3425) — its `FAMILY HISTORY` section enumerates leukemia diagnoses in relatives, so the lexical overlap outranks the actual index-case notes. This is the retrieval failure mode worth knowing about: a bi-encoder without instruction tuning can't tell "patient was diagnosed with X" from "patient's relative was diagnosed with X".
- **Rank 3–6 @ 0.613**: the target `Lymphoblastic Leukemia - Consult` cross-filed under Hematology-Oncology, General Medicine, Cardiovascular/Pulmonary, and Consult-H&P. The same four docs then re-cluster on `CHIEF COMPLAINT`, `HISTORY OF PRESENT ILLNESS`, and `LABORATORY DATA`, occupying roughly half the top-50 slots.
- Remaining hits are clinically adjacent: `Removal of Venous Port` (0.599), `Aplastic Anemia Followup`, `Non-Hodgkin lymphoma Followup`, `Polycythemia Vera Followup`, `Leiomyosarcoma`, `MediPort Placement` / `Ommaya reservoir` (chemo access), `Astrocytoma`, and a couple of general discharge summaries.

Takeaways: (1) MedTE + mean pooling retrieves a clinically coherent oncology neighborhood (leukemia → related heme/onc consults → chemo access devices), but (2) instruction-style query phrasing and the family-history-vs-index-case distinction both trip the encoder — the top hit being a family-history surface match is expected bi-encoder behavior, and a reranker stage (cross-encoder) or an instruction-tuned embedder (`intfloat/e5-*`, BGE) would be needed to fix it. (3) MTSamples cross-files the same underlying note under multiple specialty tags, so downstream retrieval should dedupe on a content hash (or on `sample_name` + `description`) before presenting results, or the top-k will be padded with duplicates.
